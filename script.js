...
function renderChartBars() {
  ...
  chartData.forEach(item => {
    ...
    const color = item.type === 'recycle' ? 'var(--success)' : 'var(--danger)';
    row.innerHTML = `
      <div class="chart-row-label">${item.label}</div>
      <div class="chart-row-bar-wrap">
        <div class="chart-row-bar" style="width: ${Math.max(12, (item.value / maxValue) * 100)}%; background: ${color};"></div>
      </div>
...
function drawChart() {
  ...
  chartData.forEach((item, index) => {
    ...
    ctx.fillStyle = item.type === 'recycle' ? 'var(--success)' : 'var(--danger)';
